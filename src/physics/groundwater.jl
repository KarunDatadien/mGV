...
function update_topsoil_moisture(
    ...
    soil_moisture_new[:, :, 1:1] = soil_moisture_old .+ topsoil_moisture_new .* fraction_layer1
    soil_moisture_new[:, :, 2:2] = soil_moisture_old .+ topsoil_moisture_new .* fraction_layer2
...
function update_bottomsoil_moisture(
    soil_moisture_old, soil_moisture_max, Q_b, Q_12, E_2_t
)
    # Bottom layer is the third layer (index 3) in your 3-layer setup, for all n
    bottomsoil_moisture = soil_moisture_old[:, :, 3:3, :]    # W_2^-[n], shape (204, 180, 1, nveg+1)
    ...
    bottomsoil_moisture_new_veg = bottomsoil_moisture[:, :, :, 1:end-1] .+ (Q_12[:, :, :, 1:end-1] .- Q_b[:, :, :, 1:end-1] .- E_2_t)
